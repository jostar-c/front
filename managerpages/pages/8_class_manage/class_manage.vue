      ...
      opendelstu(i) {
        ...
        showclass(){//展示学生
          ...
          this.students.splice(0, this.students.length);
                axios.get('../../static/students.json',{umajor:this.umajor,grade:this.grade,uclass:this.uclass})
                .then(function(response) {
                  console.log(response.data);
                  //that.stucnt=response.data[count];
                    for(var i=response.data.students.length-1;i>=0;i--)
                    {
                      that.students.push(
                       {
      ...
      delstu(i){//删除学生
        const that=this;
          this.students.splice(i, 1);//前端模拟删除
                axios.get('../../static/students.json',{sno:that.students[i].id,umajor:this.umajor,grade:this.grade,uclass:this.uclass})
                .then(function(response) {
                  //删除数据库照片
                });
        },
